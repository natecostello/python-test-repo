...
# Stub for _check_makeplot - not needed for SRS generation
def _check_makeplot(makeplot):
    """
    Stub function to replace pyyeti.ytools._check_makeplot.
    Returns None to disable plotting functionality.
    """
    return None
...
def _make_h(freq, w, bw, pass_zero, mag, nyq):
    ...
    for _w, _bw in zip(w, bw):
        ...
        if i > j or j - i < start_idx or j - i + n > freq.shape[0]:
        ...
        H[start_idx : j - i] = ramp[0]
        start_idx = j - i + n
        H[j - i : start_idx] = ramp
        on = not on
    H[start_idx:] = ramp[-1]
    return H
